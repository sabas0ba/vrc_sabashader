...
half4 frag(v2f i, bool isFront : SV_IsFrontFace) : SV_Target
{
    ...
    float2 refractionOffset = V.xy * (grazing * _RefractionStrength * _RefractionScale);
    ...
    half surfaceLight = lerp(1.0, lerp(0.35, 1.0, diffuse) * sd.shadow, saturate((half)_SurfaceShadowEnabled));
