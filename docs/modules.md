...
モジュールはシェーダー本体へ効果を追加する `.scmodule` です。SabaShader には 10 種類あり、用途ごとに個別ページへ分けています。
...
「高度モジュール」はパッケージ上の種別ではありません。10 種類は同じ `.scmodule` の仕組みで追加します。

## 有効にする

Shader Core はシェーダーごとに有効なモジュールを持ちます。本パッケージのモジュールはシェーダーと別ディレクトリにあるため、Shader Core の Project Settings で対象シェーダーへ明示的に追加します。有効にすると、マテリアルのインスペクタにモジュールの設定欄が現れます。

多くのモジュールは `Amount = 0` で無効になります。Transition は `Progress = 1` が完全表示状態です。Surface Overlay では `Droplets` を設定すると `Amount = 0` でも粒による法線の歪みが残るため、粒まで止める場合は `Droplets = 0` にしてください。
...
Decal がアルベドへ画像を合成し、Surface Detail が微細な法線と粗さを変更し、Mochi Skin が接触による法線と頂点を変形します。Spatial Interior はライティング後の色を置き換え、Transition は最後に境界と clip を適用します。効果を重ねると描画負荷が増えるため、Quest では対象マテリアルと画面占有面積を限定してください。
