...
本体が描き終えた色に、走査線・シャドウマスク・ロールバー・ざらつき・
周辺の落ち込みと、映像が乱れたときの帯のずれ・色ずれを重ねます。
頂点を帯ごとに横へずらす「裂け」も持っています。
...
隣接ピクセルを読めないためです。ずらしが要るところ（帯のずれ・色ずれ）は、
勾配 (`ddx`/`ddy`) からの 1 次近似で「ずらした先の色」を推定しています。

この近似には次の性質があります。

- 面の上で色が滑らかに変わるところではよく合う
- シルエットや模様の境目では大きく外れる。色ずれはそこで**消える**ように
  弱めてある（そうしないと縁に色の輪が残る）。乱れた帯は硬い破綻が欲しい
  効果なので、上限で頭打ちにするだけで縁でも切らない
...
なお **ブラウン管の丸みは出せません**。画面を撮り直せないので、
モデルを歪めることになり、他のオブジェクトとの位置関係が壊れます。
...
![ざらつきだけ](../tests/golden/crt_noise.png)

| プロパティ | 説明 |
| --- | --- |
| Roll Bar / Roll Speed | 垂直同期がずれた画面に出る、ゆっくり流れる明るい帯 |
| Noise / Grain Size | 画面に散らす粒と、その大きさ（画面ピクセル） |
...
![乱れた帯だけ。横にずれ、帯によっては色が入れ替わる](../tests/golden/crt_glitch.png)

| プロパティ | 説明 |
| --- | --- |
| Glitch | 帯の出やすさ。`0` で 1 本も出ず、`1` ですべての帯が出る |
| Band Height / Band Shift | 帯の高さと横ずれの幅（どちらも画面ピクセル） |
| Channel Swap | 帯で RGB を入れ替える量 |
| Vertex Tearing / Tear Band Height | 頂点を帯ごとに横へずらす幅と、帯の高さ（どちらもメートル） |

`Glitch` はピクセル側の帯と頂点の裂けの両方を出します。
...
| 表現 | 効かせるもの |
| --- | --- |
| ブラウン管 | Scanlines + Shadow Mask + Vignette、Noise はごく弱く |
| 砂嵐まじりの古い映像 | 上に Noise と Roll Bar を足す |
| 通信の乱れ | Glitch + Band Shift + Channel Swap、Scanlines は弱め |
